  ...
  while (file) {
    if (file.isDirectory()) {
      ...
      if (levels) {
        ...
        char fullPath[255]; // Adjust size as needed
        if (strcmp(dirname, "/") == 0) {
          // If we're in the root directory, just add the directory name
          sprintf(fullPath, "/%s", file.name());
        } else {
          // Otherwise, construct the path properly with the parent path
          sprintf(fullPath, "%s/%s", dirname, file.name());
        }
        listDir(fs, fullPath, levels - 1);
      }
    } else {
      Serial.print("  FILE: ");
...
#define IMAGE_WIDTH 91
#define IMAGE_HEIGHT 91
#define IMAGE_FORMAT ARGB1555
#define IMAGE_STRIDE 182
#define IMAGE_ADDR 92100  // Start address in RAM_G
